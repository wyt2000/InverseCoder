...
def get_yes_prob(probs, token_ids):
    yes_logprob = get_tokens_logprob(probs, ['YES','Yes'], token_ids)
    no_logprob = get_tokens_logprob(probs, ['NO', 'No'], token_ids)
...
def sample(llm, token_ids, sampling_params, prompts, save_path):
    ...
    with jsonlines.open(save_path, mode='a') as writer:
        for x in outputs:
            ...
            yes_prob = get_yes_prob(yes_prob, token_ids)
...
def main(
    ...
    token_ids = get_token_ids(model_path, ['YES','Yes','NO','No'])
