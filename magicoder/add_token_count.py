...
def map_dataset(
    ...
    input_ids = [
        input_id for input_id in untruncated_input_ids
    ]
...
def train():
    ...
    input_path = args.datafile_paths[0]
    save_path = f'{input_path}.token_count'
    with open(input_path, 'r') as f:
        input_lines = list(f.readlines())
    print(f'len(input_ids): {len(input_ids)}')
    print(f'len(input_lines): {len(input_lines)}')

    with jsonlines.open(save_path, mode='w') as writer:
        for line, ids in zip(input_lines, input_ids):
            line = eval(line)
            line['instruction'] = redecode(line['instruction'])
            line['response'] = redecode(line['response'])
            line['token_count'] = len(ids)
            writer.write(line)
