...
bad_codes = []
path = 'dataset/evol-instruct-gpt4/data-evol_instruct-decontaminated.jsonl.badcode.no_begin_end.no_sql'
with open(path) as f:
    lines = list(f.readlines())
    with jsonlines.open(f'{path}.three_quota', mode='w') as writer:
        for line in lines:
            line = eval(line)
    ...
    with jsonlines.open(f'{path}.no_three_quota', mode='w') as writer:
        for line in lines:
            line = eval(line)
            inst = redecode(line['instruction'])
            resp = redecode(line['response'])
